	...
	$.extend({ tablesorterPager: new function() {
		...
		renderAjax = function(data, table, p, xhr, exception){
			...
			if ( typeof(p.ajaxProcessing) === "function" ) {
				...
				ts.showError(table);
				//if(data.total_rows == 0) exception = true;
				if ( exception ) {
					if (c.debug) {
					...
					if(data.total_rows == 0) ts.showError(table,'ไม่พบข้อมูล');
					// process ajax object
					if (!$.isArray(result)) {
						p.ajaxData = result;
		...
		getAjax = function(table, p){
			...
			if ( url !== '' ) {
				...
				dataPost.search = p.dataSearch;
				p.ajaxObject.data = JSON.stringify(dataPost);
		...
		getAjaxUrl = function(table, p) {
			...
			arry = [];
			dataPost.sortList = null;
			dataPost.sortList = sortList;
			dataPost.size = p.size;
			if (sortCol) {
				sortCol = sortCol[1];
				len = sortList.length;
				for (indx = 0; indx < len; indx++) {
